...
func (w *S3Watcher) Watch(ctx context.Context, rootURI string) (<-chan FileChange, <-chan error, error) {
	initial, err := w.scanner.Scan(ctx, rootURI)
	if err != nil {
		return nil, nil, err
	}
	...
	go func() {
		...
		for {
			...
			case <-ctx.Done():
				return
			case <-ticker.C:
				current, err := w.scanner.Scan(ctx, rootURI)
				if err != nil {
